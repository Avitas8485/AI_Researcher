...
class Logger(metaclass=Singleton):
    ...
    def setup_logging(self, log_level=logging.DEBUG, log_time_format='%Y-%m-%d %H:%M:%S', theme=None):
        if theme is None:
            theme = {
        ...
        console = Console(log_time=True, log_time_format=log_time_format, theme=Theme(theme))
        ...
        rh = RichHandler(show_time=True, omit_repeated_times=False, show_level=True, show_path=False, markup=False, rich_tracebacks=True, log_time_format=log_time_format, level=log_level, console=console)
...
logger = Logger(logger_name='AT_Researcher').logger
